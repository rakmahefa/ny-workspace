...
pub fn parse_tool_calls(text: &str) -> (String, Vec<ParsedToolCall>) {
    ...
    // Parse the component structurally instead of relying on attribute order,
    // quoting style, or a single-line regex match.
...
/// Extract at most one valid FollowUp component and remove every valid
/// component from the clean assistant text. The first valid FollowUp wins,
/// matching Gemini's single-next-step semantic.
fn extract_follow_up(text: &str) -> (String, Option<(String, String)>) {
    ...
    while let Some(relative_start) = text[cursor..].find(MARKER) {
        ...
        let Some(relative_end) = find_tag_end(&text[start + MARKER.len()..]) else {
            // Incomplete components are not silently destroyed by the runtime
            // parser; the stream normalizer owns hiding them during streaming.
        ...
        if let Some((label, query)) = parse_follow_up_tag(tag) {
            ...
            // Remove valid FollowUps from normal assistant text regardless of
            // whether this was the first or an extra suggestion.
        } else {
            clean.push_str(tag);
        }
