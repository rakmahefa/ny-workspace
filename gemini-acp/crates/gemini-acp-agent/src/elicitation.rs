...
use agent_client_protocol::schema::v1::{
    ...
    ElicitationContentValue, ElicitationFormCapabilities, ElicitationFormMode,
    ElicitationId, ElicitationPropertySchema, ElicitationSchema, ElicitationUrlCapabilities,
    ElicitationUrlMode, ElicitationSessionScope, SessionId,
};
...
impl ElicitationSupport {
    pub fn from_client_capabilities(capabilities: Option<&AcpElicitationCapabilities>) -> Self {
        Self {
    ...
    pub fn can_use_form(self) -> bool { self.form }
    pub fn can_use_url(self) -> bool { self.url }
}

#[derive(Debug, Clone)]
pub struct ElicitationState(pub Arc<RwLock<ElicitationSupport>>);

impl Default for ElicitationState {
    fn default() -> Self { Self(Arc::new(RwLock::new(ElicitationSupport::default()))) }
}

impl ElicitationState {
    pub async fn get(&self) -> ElicitationSupport { *self.0.read().await }
    pub async fn set(&self, support: ElicitationSupport) { *self.0.write().await = support; }
}
...
impl ElicitationRequestSpec {
    ...
    fn into_acp_request(self) -> CreateElicitationRequest {
        ...
        CreateElicitationRequest::new(
            ElicitationFormMode::new(ElicitationSessionScope::new(self.session_id), schema),
            self.message,
        )
    }
}
...
impl std::fmt::Display for ElicitationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnsupportedMode(mode) => write!(f, "client does not support {mode} elicitation"),
            Self::InvalidUrl(url) => write!(f, "invalid elicitation URL: {url}"),
            Self::Transport(error) => write!(f, "ACP elicitation transport failed: {error}"),
        }
    }
}
...
pub async fn request_elicitation(
    ...
    if !support.can_use_form() { return Err(ElicitationError::UnsupportedMode("form")); }
    let response = cx.send_request(request.into_acp_request()).block_task().await
        .map_err(|e| ElicitationError::Transport(e.to_string()))?;
    Ok(response_to_outcome(response))
}

pub async fn request_url_elicitation(
    cx: &ConnectionTo<Client>,
    support: ElicitationSupport,
...
) -> Result<UrlElicitationOutcome, ElicitationError> {
    if !support.can_use_url() { return Err(ElicitationError::UnsupportedMode("URL")); }
    let url = url.into();
    if !(url.starts_with("https://") || url.starts_with("http://")) {
        return Err(ElicitationError::InvalidUrl(url));
    }
    let request = CreateElicitationRequest::new(
        ElicitationUrlMode::new(ElicitationSessionScope::new(session_id.clone()), elicitation_id, url),
        message,
    );
    let response = cx.send_request(request).block_task().await
        .map_err(|e| ElicitationError::Transport(e.to_string()))?;
    Ok(match response_to_outcome(response) {
        ElicitationOutcome::Accepted(content) => UrlElicitationOutcome::Accepted(content),
        ElicitationOutcome::Declined => UrlElicitationOutcome::Declined,
        ElicitationOutcome::Cancelled => UrlElicitationOutcome::Cancelled,
    })
}
...
pub async fn elicit_clarification(
    ...
    required: Vec<String>,
) -> Result<Option<BTreeMap<String, ElicitationContentValue>>, ElicitationError> {
    match request_elicitation(cx, support, ElicitationRequestSpec { session_id: session_id.clone(), message: message.to_string(), properties, required }).await? {
...
#[derive(Debug, Clone, PartialEq)]
pub enum UrlElicitationOutcome {
    Accepted(BTreeMap<String, ElicitationContentValue>),
    Declined,
    Cancelled,
}
...
fn elicitation_value_to_json(value: &ElicitationContentValue) -> serde_json::Value {
    match value {
        ElicitationContentValue::String(v) => serde_json::Value::String(v.clone()),
        ElicitationContentValue::Boolean(v) => serde_json::Value::Bool(*v),
        ElicitationContentValue::Number(v) => serde_json::json!(v),
        ElicitationContentValue::StringArray(v) => serde_json::json!(v),
...
mod tests {
    ...
    fn capability_mapping_is_exact() {
        ...
        let support = ElicitationSupport::from_client_capabilities(Some(&caps));
        assert!(support.form && support.url);
    }

    #[test]
    fn missing_capability_means_no_support() {
        let support = ElicitationSupport::from_client_capabilities(None);
        assert!(!support.form && !support.url);
    }
