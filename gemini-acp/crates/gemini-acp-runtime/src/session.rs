...
//!
//! Cette couche reprend les responsabilités de session de `claude-agent-acp`
//! dans une forme native Rust : validation stricte des identifiants et du
//! workspace, titre stable, CRUD, fork, changement de mode et annulation.
//! La persistance et la sérialisation des tours restent dans [`Store`].
...
impl SessionManager {
    ...
    pub async fn create(
        ...
        for directory in &additional_directories {
            Self::validate_cwd(directory)
                .await
                .with_context(|| format!("répertoire additionnel invalide: {}", directory.display()))?;
        }
