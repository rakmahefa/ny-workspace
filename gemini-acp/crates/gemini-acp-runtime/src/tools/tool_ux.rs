...
use agent_client_protocol::schema::v1::{
    Content, ContentBlock, Diff, SessionId, TextContent, ToolCallContent, ToolCallLocation,
...
fn file_write(args: &serde_json::Value, cwd: &Path) -> ToolInfo {
    ...
    let diff = Diff::new(resolved.clone(), content.to_owned()).old_text(old_text);
    ToolInfo {
...
fn file_edit(args: &serde_json::Value, cwd: &Path) -> ToolInfo {
    ...
    let resolved = resolve_path(path, cwd);
    let diff = Diff::new(resolved.clone(), new.to_owned()).old_text(if old.is_empty() { read_old_text(&resolved) } else { Some(old.to_owned()) });
    ToolInfo {
...
fn shell_result(result: &str, is_ok: bool, terminal_id: Option<&str>) -> ResultUpdate {
    let content = terminal_id
        .map(|id| vec![ToolCallContent::Terminal(agent_client_protocol::schema::v1::Terminal::new(id.to_owned()))])
        .unwrap_or_else(|| text_content(&format!("```console\n{}\n```", result.trim_end()), !is_ok));
    ResultUpdate {
        status: if is_ok { ToolCallStatus::Completed } else { ToolCallStatus::Failed },
        content,
        locations: vec![],
...
fn concise_args(args: &serde_json::Value) -> String {
    ...
    format!("Arguments: {keys}")
}

/// Exposed for tests and callers that only need the risk classification.
...
/// Common lifecycle formatter used by the executor.
pub fn lifecycle_title(name: &str, status: ToolCallStatus) -> String {
    match status {
        ToolCallStatus::Pending => format!("Preparing {name}"),
        ToolCallStatus::InProgress => format!("Running {name}"),
        ToolCallStatus::Completed => format!("Completed {name}"),
        ToolCallStatus::Failed => format!("Failed {name}"),
        ToolCallStatus::Cancelled => format!("Cancelled {name}"),
        _ => name.to_owned(),
    }
}
...
mod tests {
    ...
    fn lifecycle_names_are_stable() {
        assert_eq!(lifecycle_title("Read file", ToolCallStatus::Completed), "Completed Read file");
    }
}
