...
//!
//! The executor follows a strict internal lifecycle:
//!
//! `pending -> permission -> executing -> completed|failed|cancelled`
//!
//! ACP v1 deliberately has no `cancelled` tool-call status. Internal
//! `Cancelled` is therefore projected to wire-level `Failed` and explained in
//! `_meta`, matching the extensibility model used by the Claude ACP adapter.
...
use super::lifecycle::{ToolLifecycle, ToolLifecycleState};
...
impl<'a> ToolExecutor<'a> {
    ...
    pub async fn execute(&self, tool_name: &str, arguments: &Value) -> ToolResult {
        ...
        if needs_permission {
            ...
            self.emit_lifecycle(&call_id, &lifecycle, None);
        ...
        let outcome = if tool_name == "shell_exec" {
            match self.execute_shell_via_acp_terminal(arguments, &call_id).await {
                Ok(outcome) => outcome,
                Err(error) => {
                    // IMPORTANT: only terminal creation/setup errors may fall back.
                    // Once a terminal exists, execution errors must not re-run the command.
                    tracing::debug!(
        ...
        let next_state = if outcome.result.is_ok {
        ...
        lifecycle
            ...
            .expect("executing must finish in completed or failed");
        ...
        let mut meta = lifecycle_meta(tool_name, &lifecycle, None, outcome.terminal_meta);
        self.emit_update(
            &call_id,
            lifecycle.state().wire_status(),
            rendered.content,
            rendered.locations,
            Some(std::mem::take(&mut meta)),
        );
        outcome.result
    }
    ...
    async fn execute_shell_via_acp_terminal(
        ...
        // Before terminal creation, an ACP transport error is recoverable by the
        // existing local shell tool. After this point, never execute twice.
        ...
        let wait = WaitForTerminalExitRequest::new(
        ...
        let (exit_code, signal, wait_error) = match wait_result {
            Ok(Ok(wait_response)) => (
                wait_response.exit_status.exit_code,
                wait_response.exit_status.signal,
                None,
            ),
        ...
        let is_ok = wait_error.is_none() && exit_code.unwrap_or(0) == 0;
        let terminal_meta = terminal_lifecycle_meta(
            &terminal_id.0.to_string(),
            Some(&terminal_text),
    ...
    pub async fn request_permission(
        ...
        match response.outcome {
            ...
            _ => PermissionResult::TransportError(
                "outcome de permission ACP non reconnu".into(),
            ),
        }
    }
...
fn lifecycle_meta(
    ...
    meta.insert(
        ...
        json!({
            "lifecycle": {
                "state": lifecycle_state_label(lifecycle.state()),
...
mod tests {
    use super::*;
    use crate::tools::lifecycle::{ToolLifecycle, ToolLifecycleState};
    ...
    #[test]
    fn cancelled_is_encoded_as_failed_with_semantic_metadata() {
        let mut lifecycle = ToolLifecycle::new();
        lifecycle.transition(ToolLifecycleState::Permission).unwrap();
        lifecycle.cancel().unwrap();
        let meta = lifecycle_meta("shell_exec", &lifecycle, Some("cancelled"), None);
        assert_eq!(
            lifecycle.state().wire_status(),
            ToolCallStatus::Failed
        );
        assert_eq!(meta["claudeCode"]["nonExecutionKind"], "cancelled");
        assert_eq!(meta["geminiAcp"]["lifecycle"]["state"], "cancelled");
    }

    #[test]
    fn terminal_metadata_contains_protocol_compatible_keys() {
        let meta = terminal_lifecycle_meta(
            "term-1",
            Some("hello"),
            Some((Some(0), None)),
        );
